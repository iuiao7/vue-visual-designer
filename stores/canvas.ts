...
export const useCanvasStore = defineStore('canvas', () => {
  ...
  const addComponent = (componentData: BaseComponentOptions) => {
    const newComponent: BaseComponent = {
      ...
      transform: {
        x: componentData.transform?.x ?? 100,
        y: componentData.transform?.y ?? 100,
        width: componentData.transform?.width ?? 200,
        height: componentData.transform?.height ?? 100,
        rotation: componentData.transform?.rotation ?? 0,
        scaleX: componentData.transform?.scaleX ?? 1,
        scaleY: componentData.transform?.scaleY ?? 1,
      },
      zIndex: canvasState.value.components.length,
      locked: componentData.locked ?? false,
      visible: componentData.visible ?? true,
      opacity: componentData.opacity ?? 1,
